function [set_of_tokID, nr_token_av] = tokenAnyColor(...
    placeI, nr_tokens_wanted, t_color)
% function [set_of_tokID, nr_token_av] = tokenAnyColor(...
%    placeI, nr_tokens_wanted, t_color)
%
% Select tokens with **any** of the specific colors (t_color) from 
%    a specific place (placeI); selected tokens must have at least one of
%    the specified color
% Returned: Set of tokIDs will be returned. 
% 

% renamed from "select_token_with_colors"
...
while and((nr_token_av < nr_tokens_wanted), ...
    ...
    if any(ismember(t_color, token_color))
