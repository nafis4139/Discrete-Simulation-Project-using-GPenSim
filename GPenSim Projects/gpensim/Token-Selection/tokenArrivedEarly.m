function [set_of_tokID, nr_token_av] = tokenArrivedEarly(placeI, ...
                                               nr_tokens_wanted)
% function "token-Arrived-Earliest"
% [set_of_tokID,  nr_token_av] = tokenArrivedEarly(placeI, ...
%                                       nr_tokens_wanted)
%
% Selection of tokens based on their arrival time into a specific place 
% The tokens that are arrived earliest (FCFS) are needed:
%  e.g. Select three tokens from place 'p1' among the earliest arrivals: 
%          tokenArrivedEarly('p1', 3)   
%
...
[cts, sinds] = sort(token_cts, 2, 'ascend');
